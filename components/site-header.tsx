...
export function SiteHeader() {
  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-50 border-b border-[rgb(var(--line))] bg-[rgb(var(--paper))]/95 backdrop-blur-sm">
        <div className="page-shell flex h-20 items-center justify-between gap-6">
          ...
          <nav aria-label="Main navigation" className="relative flex items-center gap-3 text-sm text-[rgb(var(--ink-muted))] sm:gap-8">
            <div id="mobile-navigation" className={`${menuOpen ? 'absolute inset-x-4 top-14 z-10 flex' : 'hidden'} flex-col gap-1 border border-[rgb(var(--line))] bg-[rgb(var(--paper))] p-3 shadow-lg sm:static sm:flex sm:flex-row sm:items-center sm:gap-8 sm:border-0 sm:bg-transparent sm:p-0 sm:shadow-none`}>
